...
- **Series: rate-limit failover** — pick any codespaces (across accounts)
  ...
  `/series_schedule`).
- **Series error alerts + one-tap replace** — if a series codespace fails
  with anything that is *not* a rate limit (SSH failure, codespace deleted,
  bad permissions, unavailable machine type, out of disk, ...), the bot
  **auto-skips to the next codespace** so the rotation never stalls, and
  sends you the **exact error text** on Telegram together with buttons:
  *🔁 Replace with another codespace*, *🗑 Remove from series*, *🔄 Retry
  now* and *⏹ Stop series*. Replacing swaps in any other tracked codespace
  **in the same position** of the rotation. Same thing from the keyboard with
  `/series_replace <alias> <old> <alias> <new>` and `/series_next`.
- **Series-wide startup commands** — besides per-codespace startup
  commands, the series itself has a startup directory + command list that
  runs on **every** codespace in the series each time it boots, including
  any codespace you swap in later. Set it from *Series → ⚙️ Series startup
  commands*, or with `/series_set_dir` and `/series_set_startup`. A
  codespace's own commands run first, then the series commands.
...
| `/series_replace <alias> <old> <alias> <new>` | Swap a failing codespace for another one, same position in the rotation |
| `/series_next` | Stop the active codespace and switch to the next one |
| `/series_set_startup <cmd1;cmd2>` | Startup commands for **every** codespace in the series (incl. replacements) |
| `/series_set_dir <dir>` | Directory the series startup commands `cd` into (`-` for home) |
...
- The series loop pings only the **active** codespace of the series (every
  ...
  commands included — its own, then the series-wide ones) — rotating in a
  circle. Any **other** error (not a rate limit) does the same rotation
  *and* alerts you: the failing codespace is stopped, the next one boots
  immediately, and you get the exact error text with replace / remove /
  retry buttons (the error stays pinned in the `/series` menu until you act
  on it). If the series has only one codespace there is nothing to skip to,
  so the bot backs off and keeps retrying it. Alerts go to `OWNER_IDS`; if
  that is empty, they go to the chat you last started the series from. If a series codespace has an
