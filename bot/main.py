...
async def post_init(app: Application) -> None:
    ...
    await db.init()
    # Let background loops (series watchdog) message the owner directly.
    keeper.bot = app.bot
    ...
    await app.bot.set_my_commands(
        [
            ...
            BotCommand("series_replace", "<alias> <old> <alias> <new> swap one"),
            BotCommand("series_next", "Skip to the next codespace"),
            BotCommand("series_set_startup", "<cmd1;cmd2> for every series member"),
            BotCommand("series_set_dir", "<dir> for the series startup commands"),
            BotCommand("series_clear_startup", "Clear the series startup commands"),
