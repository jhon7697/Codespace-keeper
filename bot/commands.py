...
HELP_TEXT = (
    ...
    "/series_replace &lt;alias&gt; &lt;old&gt; &lt;alias&gt; &lt;new&gt; — swap a codespace, same position\n"
    "/series_next — stop the active codespace and switch to the next one\n\n"
    "<b>Series startup commands (every member, incl. replacements)</b>\n"
    "/series_set_startup &lt;cmd1;cmd2&gt; — run on EVERY codespace in the series\n"
    "/series_set_dir &lt;dir&gt; — directory those commands cd into\n"
    "/series_clear_startup — clear the series startup commands\n\n"
    "<b>Info</b>\n"
...
async def cmd_series(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if ser_cmds:
        ...
        lines.append(
            f"\n\u2699\ufe0f Series startup commands{where} "
            "(run on every member, incl. replacements):"
        )
    ...
    if err_cs and err_text:
        err = await db.get_codespace(err_cs)
        err_name = (err.get("display_name") or err["name"]) if err else "(deleted)"
        lines.append(
            f"\n\u26a0\ufe0f <b>Problem with {html.escape(err_name)}</b> "
            "(not a rate limit \u2014 already skipped to the next one):"
        )
        lines.append(f"<pre>{html.escape(err_text[-600:])}</pre>")
        lines.append(
            "Fix it with <code>/series_replace &lt;alias&gt; "
            f"{html.escape(err.get('name') if err else 'old')} &lt;alias&gt; "
            "&lt;new&gt;</code>, /series_next or /series (buttons)."
        )
    lines.append(
        "\nWhen the active codespace replies with a GitHub rate-limit "
        "error, I stop it and start the next one — looping forever. Any "
        "other error also skips to the next codespace automatically, and you "
        "get the exact error message plus a replace button. Auto start/stop "
...
async def cmd_series_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not await _guard(update, settings):
        return
    # Remember this chat for background alerts (used when OWNER_IDS is empty).
...
async def cmd_series_replace(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Swap a problematic codespace for another one, same position."""
    settings, db, gh, keeper = _ctx(context)
    if not await _guard(update, settings):
        return
    args = context.args or []
    if len(args) != 4:
        await _send(
            update,
            "Usage: <code>/series_replace &lt;alias&gt; &lt;old-name&gt; "
            "&lt;alias&gt; &lt;new-name&gt;</code>\n"
            "The replacement keeps the same position in the rotation and "
            "inherits the series startup commands.",
        )
        return
    old_alias, old_name, new_alias, new_name = args
    old_account = await _find_account(db, update, old_alias)
    if not old_account:
        return
    new_account = await _find_account(db, update, new_alias)
    if not new_account:
        return
    old_cs = await db.get_codespace_by_name(old_account["_id"], old_name)
    if not old_cs:
        await _send(update, f"\u274c <code>{html.escape(old_name)}</code> is not tracked.")
        return
    new_cs = await _ensure_tracked(update, db, gh, new_account, new_name)
    if not new_cs:
        return
    ok = await keeper.replace_in_series(old_cs["_id"], new_cs["_id"])
    if not ok:
        await _send(
            update,
            "\u274c Could not replace: the old codespace is not in the series, "
            "or the new one already is.",
        )
        return
    await _send(
        update,
        f"\U0001f501 Replaced <code>{html.escape(old_cs['name'])}</code> with "
        f"<code>{html.escape(new_cs['name'])}</code> in the series. The "
        "series startup commands apply to it automatically.",
    )


async def cmd_series_next(update: Update, context: ContextTypes.DEFAULT_TYPE):
    settings, db, gh, keeper = _ctx(context)
    if not await _guard(update, settings):
        return
    nxt = await keeper.skip_to_next()
    if not nxt:
        await _send(update, "\u274c The series is empty.")
        return
    cs = await db.get_codespace(nxt)
    name = (cs.get("display_name") or cs["name"]) if cs else nxt
    await _send(update, f"\u23ed Switched to <b>{html.escape(str(name))}</b>.")


async def cmd_series_set_startup(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Startup commands that run on EVERY codespace in the series."""
    settings, db, gh, keeper = _ctx(context)
    if not await _guard(update, settings):
        return
    ...
    if not raw:
        await _send(
            update,
            "Usage: <code>/series_set_startup &lt;cmd1;cmd2&gt;</code>\n"
            "Separate multiple commands with <code>;</code>. They run on "
            "every codespace in the series each time it starts \u2014 "
            "including codespaces swapped in later.",
        )
        return
    commands = [part.strip() for part in raw.split(";") if part.strip()]
    await db.save_series({"startup_commands": commands})
    listed = "\n".join(f"\u2022 <code>{html.escape(c)}</code>" for c in commands)
    series = await db.get_series()
    workdir = series.get("startup_dir")
    where = (
        f"<code>{html.escape(workdir)}</code>" if workdir else "the home directory"
    )
    await _send(
        update,
        f"\u2705 Series startup commands saved ({len(commands)}), running in "
        f"{where} on every codespace in the series:\n{listed}",
    )


async def cmd_series_set_dir(update: Update, context: ContextTypes.DEFAULT_TYPE):
    settings, db, gh, keeper = _ctx(context)
    if not await _guard(update, settings):
        return
    raw = " ".join(context.args or []).strip()
    if not raw:
        await _send(
            update,
            "Usage: <code>/series_set_dir &lt;directory&gt;</code> (e.g. "
            "<code>~/mydirectory</code>, or <code>-</code> for the home "
            "directory)",
        )
        return
    if raw.startswith("cd "):
    ...
    saved = f"<code>{html.escape(workdir)}</code>" if workdir else "(home directory)"
    await _send(update, f"\u2705 Series startup directory: {saved}")


async def cmd_series_clear_startup(update: Update, context: ContextTypes.DEFAULT_TYPE):
    settings, db, gh, keeper = _ctx(context)
    if not await _guard(update, settings):
        return
    await db.save_series({"startup_commands": [], "startup_dir": None})
    await _send(update, "\U0001f9f9 Series startup commands cleared.")
...
def register(app: Application) -> None:
    ...
    app.add_handler(CommandHandler("series_replace", cmd_series_replace))
    app.add_handler(CommandHandler("series_next", cmd_series_next))
    app.add_handler(CommandHandler("series_set_startup", cmd_series_set_startup))
    app.add_handler(CommandHandler("series_set_dir", cmd_series_set_dir))
    app.add_handler(CommandHandler("series_clear_startup", cmd_series_clear_startup))
